...
import com.gerritforge.gerrit.globalrefdb.validation.dfsrefdb.SharedRefEnforcement.Rule;
...
public class SharedRefDbConfiguration {
  ...
  public static class SharedRefDatabase {
    ...
    private final ImmutableList<Rule> enforcementRules;
     ...
     * entry can be either be {@link SharedRefEnforcement.Rule.Policy#INCLUDE} or {@link
     * SharedRefEnforcement.Rule.Policy#EXCLUDE} and it represents the level of consistency
     * enforcements for that specific "project:refs".
     *
     ...
     */
    public List<Rule> getEnforcementRules() {
     ...
     */
    private ImmutableList<Rule> parseEnforcementRulesList(List<String> enforcementRulesList) {
      List<Rule> calculatedRules = new ArrayList<>();
      ...
      for (String item : enforcementRulesList) {
        ...
        if (parts.size() == 3) {
          ...
          try {
            calculatedRules.add(new Rule(SharedRefEnforcement.Policy.valueOf(key), project, ref));
